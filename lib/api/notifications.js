...
// Fan a notification out to everyone on a project (owner + members), including
// the actor. Goes through the notify_project SECURITY DEFINER function so it can
// write a row per recipient under RLS. Best-effort: a notification failure must
// never break the underlying action, so callers should not let this throw.
export const notifyProject = async (projectId, type, title, message, link = null) => {
  if (!projectId) return
  const { error } = await supabase.rpc('notify_project', {
