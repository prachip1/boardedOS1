...
// Upload a file to the shared `files` bucket and record it against a task.
// projectId is stored so RLS can check project membership/ownership.
export const uploadTaskAttachment = async (taskId, projectId, file) => {
  const { data: { user } } = await supabase.auth.getUser()

  const fileExt = file.name.split('.').pop()
  const fileName = `${nanoid()}.${fileExt}`
  const filePath = `${user.id}/task-attachments/${fileName}`

  // Upload to storage first, then write the metadata row.
  await uploadFile('files', filePath, file)
  ...
  if (error) {
    // Roll back the orphaned upload if the DB insert was rejected.
    try { await deleteFromStorage('files', filePath) } catch (_) { /* best effort */ }
    throw error
  }
  return data
}

// Delete an attachment (storage object + metadata row). RLS limits who can.
export const deleteTaskAttachment = async (id) => {
  ...
  if (row?.storage_path) {
    try { await deleteFromStorage('files', row.storage_path) } catch (_) { /* best effort */ }
  }
}
